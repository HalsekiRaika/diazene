...
impl<A: Actor> ActorRef<A> {
    pub(crate) fn new(sender: UnboundedSender<Box<dyn Applier<A>>>) -> ActorRef<A> {
        Self { ctx: Arc::new(RefContext { sender })  }
    }
}
...
impl<A: Actor> ActorRef<A>
{
    pub async fn ask<M: Message>(&self, msg: M) -> Result<Result<A::Accept, A::Rejection>, ActorError> where A: Handler<M> {
        ...
        let Ok(_) = self.ctx.sender.send(Box::new(Callback { message: msg, oneshot: tx })) else {
    ...
    pub async fn tell<M: Message>(&self, msg: M) -> Result<Result<(), A::Rejection>, ActorError> where A: Handler<M> {
        let (tx, rx)= oneshot::channel();
        let Ok(_) = self.ctx.sender.send(Box::new(Void { message: msg, oneshot: tx })) else {
            return Err(ActorError::CallBackSend)
        };
        let Ok(res) = rx.await else {
            return Err(ActorError::CallBackSend)
...
impl AnyRef {
    pub fn downcast<A: Actor>(self) -> Result<ActorRef<A>, ActorError> {
        Ok(ActorRef { ctx: self.0.downcast::<RefContext<A>>().map_err(|_| ActorError::DownCastFromAny)? })
    }
}
